(function () {
	...
	const DEBUG_MODE =
		window.location.search.includes('ab-debug=true') || window.location.search.includes('ab_debug=true');
	...
	const THEME_CONFIGS = {
		dawn: {
			...
			gallery: {
				containers: ['.product__media-list', 'ul.product__media-list', 'media-gallery .product__media-list'],
				items: ['.product__media-item', 'li.product__media-item', '.product__media-list > li'],
				images: ['.product__media img', '.product__media-item img', '.product-media-container img'],
				itemTemplate: 'li',
		...
		horizon: {
			...
			gallery: {
				containers: ['slideshow-slides', 'media-gallery', 'slideshow-container'],
				items: ['slideshow-slide', '.product-media-container'],
				images: ['slideshow-slide img', '.product-media__image', '.product-media img'],
	...
	function isProductImage(img) {
		...
		if (
			...
			src.includes('.myshopify.com/cdn/')
		) {
	...
	function cleanupGallery(gallery) {
		...
		themeConfig.hiding.cleanupSelectors.forEach(selector => {
			...
			elements.forEach(el => {
				const hasVisible = Array.from(el.children).some(
					child => child.dataset.abTestHidden !== 'true' && child.style.display !== 'none',
				);
